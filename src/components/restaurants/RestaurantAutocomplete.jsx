...
const DiscoverPage = () => {
  ...
  ><Filter className="h-4 w-4 mr-2" />
            Filtros
            {Object.keys(filters).some((key) => filters[key]) && <Badge className="ml-2 bg-primary text-primary-foreground text-xs">
                Activos
              </Badge>}</Button></div>{
    /* Filter Panel */
    ...
    onValueChange={(value) => handleFilterChange("cuisine", value || void 0)}
  ><SelectTrigger className="bg-secondary border-border"><SelectValue placeholder="Todas las cocinas" /></SelectTrigger><SelectContent><SelectItem value="">Todas las cocinas</SelectItem>{cuisines.map((cuisine) => <SelectItem key={cuisine} value={cuisine}>{cuisine}</SelectItem>)}</SelectContent></Select></div>{
    /* Location Filter */
  }<div><label className="text-sm font-medium text-foreground mb-2 block">
                    Ubicación
                  </label><Select
    value={filters.location || ""}
    onValueChange={(value) => handleFilterChange("location", value || void 0)}
  ><SelectTrigger className="bg-secondary border-border"><SelectValue placeholder="Todas las ubicaciones" /></SelectTrigger><SelectContent><SelectItem value="">Todas las ubicaciones</SelectItem>{locations.map((location) => <SelectItem key={location} value={location}>{location}</SelectItem>)}</SelectContent></Select></div>{
    /* Price Range Filter */
  }<div><label className="text-sm font-medium text-foreground mb-2 block">
                    Rango de precio
  ...
  /></div></CardContent></Card>)}</div> : <div className="text-center py-12"><div className="h-16 w-16 bg-muted rounded-full flex items-center justify-center mx-auto mb-4"><Search className="h-8 w-8 text-muted-foreground" /></div><h3 className="text-lg font-semibold text-foreground mb-2">
              ...
              Intenta ajustar tus filtros de búsqueda o explorar otras opciones
            </p><Button
    onClick={clearFilters}
